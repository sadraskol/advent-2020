------------------------------ MODULE Advent11 ------------------------------
EXTENDS Sequences, Naturals, FiniteSetsExt, Integers
...
In == Input
Preheat == [i \in 1..Len(In) |-> [j \in 1..Len(In[i]) |-> SubSeq(In[i], j, j)]]
...
SeatAt(i, j) == IF i < 1 \/ i > Height \/ j < 1 \/ j > Width
                THEN "L"
                ELSE seats[i][j]

OccupiedSeat(s) == IF s = "#" THEN 1 ELSE 0

FirstRule(i, j) ==
    LET neighbors == ({i - 1, i, i + 1} \X {j - 1, j, j + 1}) \ {<<i, j>>}
        occupiedSeats == ReduceSet(LAMBDA s, acc: acc + OccupiedSeat(SeatAt(s[1], s[2])), neighbors, 0)
     IN CASE SeatAt(i, j) = "L" /\ occupiedSeats = 0 -> "#" 
          [] SeatAt(i, j) = "#" /\ occupiedSeats >= 4 -> "L"
          [] TRUE -> SeatAt(i, j)

Directions == ({-1, 0, 1} \X {-1, 0, 1}) \ {<<0, 0>>}

RECURSIVE SearchInDirection(_,_,_)
SearchInDirection(i, j, dir) ==
    IF SeatAt(i, j) = "."
    THEN SearchInDirection(i + dir[1], j + dir[2], dir)
    ELSE SeatAt(i, j)

SecondRule(i, j) ==
    LET occupiedSeats == ReduceSet(LAMBDA dir, acc: acc + OccupiedSeat(SearchInDirection(i + dir[1], j + dir[2], dir)), Directions, 0)
     IN CASE SeatAt(i, j) = "L" /\ occupiedSeats = 0 -> "#"
          [] SeatAt(i, j) = "#" /\ occupiedSeats >= 5 -> "L"
          [] TRUE -> SeatAt(i, j)          

Rule(a, b) == SecondRule(a, b)

CountOccupied(seq) ==
    LET occupiedByRow == [i \in 1..Len(seq) |-> Len(SelectSeq(seq[i], LAMBDA x: x = "#"))]
     IN ReduceSet(LAMBDA i, acc: occupiedByRow[i] + acc, DOMAIN occupiedByRow, 0)

Init == seats = Preheat

Next == seats' = [i \in 1..Len(seats) |-> [j \in 1..Len(seats[i]) |-> Rule(i, j)]]

Invariant == \E i \in 1..Len(seats): \E j \in 1..Len(seats[i]): SeatAt(i, j) /= Rule(i, j)
