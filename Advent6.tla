...
RECURSIVE Letters(_,_)
Letters(line, acc) ==
    IF Len(line) = 0
    THEN acc
    ELSE Letters(Tail(line), acc \union {Head(line)})

NULL == {"ioroihgihoergoihoihr"}

EmptyGroup == [passengers |-> 0, letters |-> NULL]

(*RECURSIVE RegroupGroups(_,_,_)
RegroupGroups(input, groups, currentGroup) ==
    IF Len(input) = 0
    THEN Append(groups, currentGroup)
    ELSE IF Len(Head(input)) = 0
         THEN RegroupGroups(
            Tail(input),
         ...
         ELSE RegroupGroups(
            Tail(input),
            groups,
            [ passengers |-> currentGroup.passengers + 1,
              letters |-> currentGroup.letters \union Letters(Head(input), {})
            ]
        )
Groups == RegroupGroups(Input, <<>>, EmptyGroup)
*)

VARIABLE i, groups, sum, currentGroup
...
Next == /\ \/ /\ i > Len(Input)
              /\ UNCHANGED <<groups, currentGroup, sum>>
           \/ /\ Input[i] = ""
              /\ groups' = Append(groups, Cardinality(currentGroup.letters))
              /\ sum' = sum + Cardinality(currentGroup.letters)
              /\ currentGroup' = EmptyGroup
           \/ /\ Input[i] /= ""
              /\ currentGroup' = [ passengers |-> currentGroup.passengers + 1,
                                   letters |->
                                    IF currentGroup.letters = NULL
                                    THEN Letters(Input[i], {})
                                    ELSE Letters(Input[i], {}) \intersect currentGroup.letters
                                 ]
              /\ UNCHANGED <<groups, sum>>
        /\ i' = i + 1

FinishCalculation == i <= Len(Input) + 1
=============================================================================
